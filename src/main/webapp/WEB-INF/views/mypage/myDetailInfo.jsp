...
function fn_changeMail(){
	if($('#selectMail').val() == 'directly'){
		$('#selboxDirect').prop("type", "text");
	} else{
		$('#selboxDirect').prop("type", "hidden");
		$('#textEmail').val($('#select').val());
	}
}

function fn_mail(){
	$('#textEmail').val($('#selboxDirect').val());
}

$(document).ready(function () {
	var exists = false;
	$('#selectMail').find('option').each(function(){
		if(this.value == '${memberInfo.email2 }'){
			exists = true;
			return false;
		}
	})
	if(exists == true){
	$('#selectMail').val('${memberInfo.email2}').prop('selected', true);
	} else{
		$('#selectMail').append("<option value='${memberInfo.email2}' selected>${memberInfo.email2}</option>");
	}
	})
</script>
</head>
			...
			<tbody>
				...
				<tr class="dot_line">
					...
					<td>
					   ...
					   <input type="hidden" id="selboxDirect" name="selboxDirect" onChange="fn_mail()"/>
					   <select name="email2" onChange="fn_changeMail()"  title="직접입력" id="selectMail">
									<option value="" disabled selected>E-Mail 선택</option>
									<option value="directly" id="textEmail">직접입력</option>
									<option value="hanmail.net" id="hanmail.net">hanmail.net</option>
									<option value="naver.com" id="naver.com">naver.com</option>
									<option value="nate.com" id="nate.com">nate.com</option>
									<option value="google.com" id="google.com">google.com</option>
